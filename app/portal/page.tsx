...
async function Portal({ searchParams }: { searchParams: { q: string } }) {
  ...
  const [session, products, subscription, users, songs, artists] =
    await Promise.all([
      getSession(),
      getActiveProductsWithPrices(),
      getSubscription(),
  ...
  const currentArtist = !adminEmails.includes(session?.user.email!)
    ? artists.artists?.find(
        (artist: { contact_email: string }) =>
          artist.contact_email === session?.user.email, //"blvckheartmoney@gmail.com" //,
      )
    : [];
  const currentArtistSongs =
    currentArtist && !adminEmails.includes(session?.user.email!)
      ? songs.songs?.filter(
          (song) => song.artist_name === currentArtist?.artist_name,
        )
      : songs.songs || [];
