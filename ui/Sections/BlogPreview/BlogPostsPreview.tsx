...
const BlogPost = ({ _createdAt, title, content, author, slug, coverImage }) => {
  ...
  return (
    <article className="p-6 bg-white rounded-lg border border-zinc-200 shadow-md dark:bg-zinc-950 dark:border-zinc-800">
      ...
      <h2 className="mb-2 text-2xl font-medium font-owners tracking-tight text-zinc-900 dark:text-white">
        <Link href={`/post/${slug?.current}`}>{title}</Link>
      </h2>
      <div className="mb-5 font-light text-zinc-500 dark:text-zinc-400 text-sm">
        <PortableText content={excerpt} shorten={true} maxLength={240} />
      </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="rounded-full bg-zinc-100 contrast-">
            <Image
              ...
              src={getSiteImage("/network-ball.png")}
              alt={`${author} avatar`}
            />
          </div>
