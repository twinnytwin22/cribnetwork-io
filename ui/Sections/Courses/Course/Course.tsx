...
function Course({ course, image }) {
  ...
  function countModulesInCourse(course: {
    lessons: { modules: string | any[] }[];
  }) {
    let totalModules = 0;
                    ...
                    {lesson?.modules?.map((mod: any) => (
                      <div
                        ...
                        className={`cursor-pointer ${
                          modules[activeModuleIndex]?.title === mod?.title
      ...
      <div className="p-8 place-items-center max-w-screen w-full min-h-[100vh] h-full text-black dark:text-white ">
        {!finishedScreen && activeModuleIndex !== null ? (
          <div className="space-y-8 ">
            ...
            <p className="hidden">
              {JSON.stringify(modules[activeModuleIndex].content)}
            </p>
          ...
          <div className="space-y-8">
            ...
            <p className="hidden">
              {JSON.stringify(lessons[activeLessonIndex])}
            </p>
            ...
            {!finishedScreen && (
              <PortableText
                content={
                  ...
                  lessons[activeLessonIndex]?.overview ||
                  ""
                }
              />
            )}
        ...
        {finishedScreen && (
          <div className="p-8 place-items-center w-full min-h-[80vh] h-full">
            <h1 className={courseHeaderStyle + "text-center"}>FINISHED</h1>
          </div>
        )}
        {!finishedScreen ? (
          <button
            onClick={handleNextClick}
            className="bg-red-300 text-black p-2 mt-4 rounded font-bold px-6"
          >
            Next
          </button>
        ) : (
          <Link
            href={`/portal/learning/course/${course?._id}`}
            className="bg-red-300 text-black font-bold p-2 mt-4 rounded px-6"
          >
            Exit
          </Link>
        )}
