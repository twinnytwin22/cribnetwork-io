            ...
            <tbody className="bg-white dark:bg-zinc-800 divide-y divide-zinc-200">
                {courses?.map((course: any) => (
                    <tr key={course.id} className="text-black dark:text-white">
                        ...
                        <td className="px-6 py-4 whitespace-no-wrap">
                            {course.enrollment_status === 'in_progress' && <span className="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">In Progress</span>
                            }                        {course.enrollment_status !== 'in_progress' && course?.enrollment_status}
                        </td>
                        <td className="px-6 py-4 whitespace-no-wrap">
