  ...
  return (
    <form className="flex items-center flex-grow relative">
      ...
      <button
        ...
        className="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-zinc-500 bg-zinc-100 border border-zinc-300 rounded-l-lg hover:bg-zinc-200 focus:ring-4 focus:outline-none focus:ring-zinc-100 dark:bg-zinc-700 dark:hover:bg-zinc-600 dark:focus:ring-zinc-700 dark:text-white dark:border-zinc-600"
        type="button"
      >
        Type
      </button>
          ...
          {searchResults?.courses?.length > 0 && (
            <div className="relative">
              ...
              {searchResults.courses.map((course: any) => (
                <div
                  onClick={() =>
                    handleLink(`/portal/learning/course/${course.id}`)
                  }
                  key={course.id}
...
const SearchButton = () => {
  return (
    <button
      type="submit"
      className="p-2.5 ml-2 text-sm font-medium text-black bg-red-300 rounded-md border border-red-300 hover:bg-red-300 focus:ring-4 focus:outline-none focus:ring-zinc-100 dark:bg-red-300 dark:hover:bg-red-300 dark:focus:ring-zinc-100"
    >
      <svg
        className="w-5 h-5"
        fill="none"
