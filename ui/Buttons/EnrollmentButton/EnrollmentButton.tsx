...
function EnrollmentButton({ course }) {
    ...
    const getEnrollmentStatus = async ({ user }) => {
        if (user && course) {
            ...
            .from('bookmarked_courses').select()
            .match({
                student_id: user?.id,
            ...
            try {
                ...
                if (!data) {
                    ...
                    return { enrollment: data.enrollment_status, bookMarkStatus: bookMarkStatus || 'false'};
                }
            } catch (error) {
    ...
    const handleBookMarkAction = async () => {//const data = await getEnrollmentStatus({user})
        try {
            if (bookMarkedStatus === 'false') {
                ...
                // .select()
                    ...
                    await supabase
                    .from('bookmarked_courses').delete()
                    .match({
                        student_id: user?.id,
                        course_id: course?._id,
                    //    enrollment_status: 'enrolled'
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["enrollment","bookMarkedStatus", user, enrolled, router, handleBookMarkAction],
...
>
  {bookMarkedStatus !== 'false' ? <FaBookmark className="text-black" /> : <FaRegBookmark />}
</button>
